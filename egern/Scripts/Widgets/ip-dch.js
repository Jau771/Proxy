export default async function(ctx) {
  ...
  if (['systemSmall', 'accessoryCircular', 'accessoryInline', 'accessoryRectangular'].includes(widgetFamily)) {
    return { type: 'widget', padding: 16, ...(isTransparentMode ? {} : { backgroundColor: BG_COLOR }), children: [{ type: 'text', text: '请使用中号或大号组件', font: { size: 'callout' }, textColor: C_MAIN, textAlign: 'center' }] };
  }
  ...
  return {
    ...
    gap: TOP_GAP,
    ...(isTransparentMode ? {} : { backgroundColor: BG_COLOR }),
    children: [
      {
        type: 'stack',
        direction: 'row',
